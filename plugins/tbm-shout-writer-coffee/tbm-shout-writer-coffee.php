<?php
/**
 * Plugin Name: Shout writer a coffee
 * Plugin URI: https://thebrag.media/
 * Description: Shout writer a coffee (via PayPal)
 * Version: 1.0.0
 * Author: Sachin Patel
 * Author URI: http://www.patelsachin.com
 */

class TBM_Shout_Writer_Coffee {
    ...
    public function __construct() {
        $this->plugin_name = 'tbm_shout_writer_coffee';
        $this->plugin_slug = 'tbm-shout-writer-coffee';

        add_action('init', array( $this, 'shout_writer_coffee_init' ) );

        add_shortcode( 'shout_writer_coffee', array( $this, 'shortcode_shout_writer_coffee_func' ) );

        add_action( 'admin_menu', array( $this, 'settings_menu' ) );
    }

    public function settings_menu() {
        add_options_page( 'Shout writer coffee', 'Shout writer coffee', 'administrator', 'tbm-shout-writer-coffee', array( $this, 'settings_page' ) );
    }
...
?>
        <form method="post" class="form">
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-12"><h3>Shout the writer a coffee - settings</h3></div>
                        <div class="col-12">
                            <div class="form-group">
                                <label>Exclude Authors</label>
                                <textarea name="tbm_shout_coffee_exclude" id="tbm_shout_coffee_exclude" class="form-control"><?php echo get_option( 'tbm_shout_coffee_exclude' ); ?></textarea>
                            </div>

                            <div class="form-group">
                                <label>&quot;About this&quot; link URL</label>
                                <input
                                    name="tbm_shout_coffee_about_url" id="tbm_shout_coffee_about_url" type="text"
                                    value="<?php echo stripslashes( get_option( 'tbm_shout_coffee_about_url' )  ); ?>"
                                    placeholder="https://" class="form-control">
                            </div>

                            <div class="form-group">
                                <label>Pledging 100% to ALS</label>
                                <textarea name="tbm_shout_coffee_pledging" id="tbm_shout_coffee_pledging" class="form-control"><?php echo get_option( 'tbm_shout_coffee_pledging' ); ?></textarea>
                            </div>
                        </div>
...
<?php
     ...
     */
    public function shout_writer_coffee_init() {
        if ( isset( $_GET['message'] ) && 'thank-you-shout-coffee' == $_GET['message'] ) :
            add_action( 'wp_footer', function () {
                echo '<div class="modal fade" id="thankYouShoutCoffeeModal" tabindex="-1" role="dialog" aria-labelledby="thankYouShoutCoffeeModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                  <div class="modal-body">
                    <div class="">
                        ...
                        <h2 class="text-center py-3 px-5">Thank you for shouting a coffee to the writer!</h2>
                    </div>
                  </div>
                </div>
              </div>
        ...
        // If Shout a coffee form is submitted
        if ( isset( $_POST['btn-shout-writer-coffee'] ) ) {
...
//            echo '<pre>'; print_r( $_POST ); exit;
            ...
            $invoice_id = $this->curl_post( 'https://thebrag.media/wp-json/api/v1/shout_writer_coffee', 'POST', $curl_metadata );

            $data_paypal = array(
                'invoice' => $invoice_id,
                'cmd' => '_xclick',
                'business' => $paypal_config['business'],
                'item_name' => 'Shout ' . $author . ' a coffee @ The Brag Media',
                'mesasge' => $message,
                'custom' => $message,
                'article' => $article_url,
                'amount' => $total,
                ...
                'lc' => 'AU',
                'currency_code' => 'AUD',
                'return' => $article_url . '?message=thank-you-shout-coffee',
...
//            echo $queryString; exit;
    ...
    public function shortcode_shout_writer_coffee_func( $atts ) {
        ...
        $exclude_authors = get_option( 'tbm_shout_coffee_exclude' );
        ...
        $about_url = stripslashes( get_option( 'tbm_shout_coffee_about_url' )  );
        ob_start();
        ?>
        <script src="https://www.google.com/recaptcha/api.js?render=<?php echo $this->recaptchaSiteKey; ?>"></script>
        <script>grecaptcha.ready(function() {
          grecaptcha.execute("<?php echo $this->recaptchaSiteKey; ?>", {action: "shout_coffee"}).then(function (token) {
            jQuery(".recaptchaResponse").val(token);
          });
        });</script>
        <div style="width: auto; max-width: 100%; margin: 0 auto;">
          <button type="button" class="btn btn-block btn-sm btn-outline-dark btn-shout-writer-coffee mt-1 mb-2 d-flex justify-content-between align-items-center" data-toggle="form-shout-writer-coffee">
            <span class="mr-1 small"><i class="fa fa-coffee mr-2"></i></span>
            <span class="text-right small">Love this article?<br>Shout <?php echo $a['author']; ?> a coffee</span>
          </button>
          <form action="" method="post" name="form-shout-writer-coffee" class="form-shout-writer-coffee d-none mb-3" target="_blank">
            <div class="input-group">
              <div class="input-group-prepend"><div class="input-group-text">$</div></div>
              <input type="number" name="amount" class="required form-control shout-writer-coffee-amount" min="5" value="5" required>
              ...
              <input type="submit" value="Proceed" name="btn-shout-writer-coffee" class="button btn btn-dark btn-shout-writer-coffee form-control">
            </div>
        ...
        $pledging_authors = get_option( 'tbm_shout_coffee_pledging' );
