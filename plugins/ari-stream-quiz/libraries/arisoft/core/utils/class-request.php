...
class Request {
    ...
    public static function root_url() {
        ...
        $protocol = Utils::get_value( $url_parts, 'scheme', 'http' );
        $is_SSL = 'https' === $protocol;
        $port = Utils::get_value( $url_parts, 'port', 0 );
        $port = ( ( $is_SSL && $port != 443 && $port != 80 ) || ( ! $is_SSL && $port != 80) ) ? $port : 0;
        $host = Utils::get_value( $url_parts, 'host', $_SERVER['SERVER_NAME'] );
/*
...
*/
        self::$root_url = $protocol . '://' . $host . ( $port > 0 ? ':' . $port : '' );
