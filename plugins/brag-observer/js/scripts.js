jQuery(document).ready(function ($) {
  ...
  $(".btn-login").on("click", function () {
    ...
    var apple_signin_elem = $("#loginModal").find(
      ".wp-social-login-provider-apple"
    );
    var apple_signin_url = apple_signin_elem.prop("href");
    apple_signin_elem.prop(
      "href",
      apple_signin_url + "&state=" + $(btn).data("apple")
    );
  });
  ...
  $("#modal-form-login").on("submit", function (e) {
    ...
    $(".js-errors-login,.js-success-login,.js-msg-login")
      .html("")
      .addClass("d-none");
    loadingElem.show();
    button.hide();
  ...
  $(".btn-subscribe-observer").on("click", function () {
    ...
    $("#subscribeObserverModal")
      .find("#modal-title-subscribe")
      .text($(btn).data("topic"));
    $("#subscribeObserverModal")
      .find("#modal-desc-subscribe")
      .text($(btn).data("desc"));
    $("#subscribeObserverModal")
      .find("#modal-list-subscribe")
      .val($(btn).data("list"));

    var apple_signin_elem = $("#subscribeObserverModal").find(
      ".wp-social-login-provider-apple"
    );
    var apple_signin_url = apple_signin_elem.prop("href");
    apple_signin_elem.prop(
  ...
  $(".btn-vote-observer").on("click", function () {
    ...
    $("#voteObserverModal")
      .find(".progress-bar")
      .css("width", (parseInt($(btn).data("votes")) * 100) / 1000 + "%");
    $("#voteObserverModal").find("#votes_count").text($(btn).data("votes"));

    var apple_signin_elem = $("#voteObserverModal").find(
      ".wp-social-login-provider-apple"
    );
    var apple_signin_url = apple_signin_elem.prop("href");
    apple_signin_elem.prop(
      ...
      $.post(brag_observer.url, data, function (res) {
        ...
        if (res.success) {
          ...
          if ("vote_observer" == v.action) {
            var votes_count =
              parseInt(btn.parent().find(".votes_count").text()) + 1;
            btn.parent().find(".votes_count").text(votes_count);
            var votes_target = parseInt(
              btn.parent().find(".votes_target").text()
            );
            btn
              .parent()
              .find(".progress-bar")
              .css("width", (votes_count * 100) / votes_target + "%");
          }
        } else {
  ...
  $("#observer-subscribe-form,#observer-subscribe-form2").on(
    "submit",
    function (e) {
      e.preventDefault();
      var theForm = $(this);
      ...
      $.post(brag_observer.url, data, function (res) {
        if (res.success) {
          theForm
            ...
            .html(
              "You will need to confirm your email address in order to activate your account. An email containing the activation link has been sent to your email address. If the email does not arrive within a few minutes, check your spam folder."
            )
            .removeClass("d-none");
          // window.location.reload();
        } else {
          if ("email_invalid" == res.data) {
            theForm
              ...
              .html("Please input valid email address.")
              .removeClass("d-none");
          } else if ("email_exists" == res.data) {
            theForm
              .find(".js-errors-subscribe")
              .html(
                'User account already exists with the provided email address, please <a href="/observer/?login">login</a> to continue'
              )
              .removeClass("d-none");
          } else if ("birthday_invalid" == res.data) {
            theForm
              .find(".js-errors-subscribe")
              .html("Please input valid birthday.")
              .removeClass("d-none");
