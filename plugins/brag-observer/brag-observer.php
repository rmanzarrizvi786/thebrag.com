...
class BragObserver
{
  ...
  */
  public function setupIntent($customer_id = NULL, $subscription = NULL)
  {
    require_once __DIR__ . '/classes/payment.class.php';
    $payment = Payment::getInstance();

    if (is_null($customer_id) || '' == $customer_id) {
      // Customer ID is not available => Create customer in Stripe

      if (is_null($customer_id) || '' == $customer_id) {
        $buyer = [
          'full_name' => $subscription->crm_record->Buyer__c,
          'address_1' => $subscription->crm_record->Address_1__c,
          'address_2' => $subscription->crm_record->Address_2__c,
          'city' => $subscription->crm_record->City__c,
          'country' => array_search($subscription->crm_record->Country__c, self::getCountries()),
          'postcode' => $subscription->crm_record->Postcode__c,
          'state' => $subscription->crm_record->State__c,
        ];
        $shipping = [
          'address_1' => $subscription->crm_record->Shipping_Address_1__c,
          'address_2' => $subscription->crm_record->Shipping_Address_2__c,
          'city' => $subscription->crm_record->Shipping_City__c,
          'country' => array_search($subscription->crm_record->Shipping_Country__c, self::getCountries()),
          'postcode' => $subscription->crm_record->Shipping_Postcode__c,
          'state' => $subscription->crm_record->Shipping_State__c,
        ];
        $customer = $payment->createCustomer(NULL, $subscription->crm_record->Email__c, $subscription->crm_record->Name, $buyer, $shipping);
        ...
        if ($customer['error']) {
          ...
          $post_data['stripe_customer_id'] = $customer_id;
          $this->updateCustomerId($post_data);
        }
      }
      // return;
    }

    return $payment->setupIntent($customer_id, $subscription);
  }

  public function updateCustomerId($data)
  {
    $data['key'] = $this->mag_sub['rest_api_key'];

    $update_json = $this->callAPI(
      'POST',
      $this->mag_sub['api_url'] . 'update_customer_id',
      $data,
      false
    );

    return json_decode($update_json);
  }
