...
class Tastemaker extends BragObserver {
  ...
  */
  public function load_js_css() {
    wp_enqueue_script( 'brag_observer', plugin_dir_url( __FILE__ ) . '../js/scripts.js', array ( 'jquery' ), time(), true);
    $args = array(
  ...
  */
  public function save_tastemaker_review( $formData = [], $is_rest = false ) {
    ...
    if ( count( $errors ) > 0 ) {
      wp_send_json_error( $errors ); wp_die();
    }
    ...
    if ( ! $check_review ) {
      ...
      wp_send_json_error( [ 'You have already provided feedback here, thank you!'] ); wp_die();
    }
