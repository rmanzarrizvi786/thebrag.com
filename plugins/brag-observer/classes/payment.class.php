  ...
  */
  ...
  public function update_payment_details($customer_id) {
    ...
    try {
      ...
      wp_send_json_error( [ 'error' => 'Error setting default payment method.' ] ); wp_die();
    }

    wp_send_json_success($setupIntent->payment_method); wp_die();
  }
  ...
  public function createCustomer( $payment_method, $sub_email, $sub_full_name, $buyer = [], $shipping = []) {
    ...
    try {
      ...
      return [ 'error' => 'It looks like your card has been declined. Make sure all your details are correct, your card is valid, and give it another shot.' ]; wp_die();
    } catch (\Stripe\Exception\RateLimitException $e) {
      error_log( '--Stripe Error | Customer | RateLimitException: ' . $e->getError()->message );
      return[ 'error' => 'Whoops, like something unexpected happened on our of things. Feel free to refresh your browser and give it another shot.' ]; wp_die();
      ...
      return[ 'error' => 'Whoops, it seems we had a bit of trouble getting in contact with the payment service. Feel free to refresh your browser and give it another shot!' ]; wp_die();;
    } catch (\Stripe\Exception\ApiErrorException $e) {
      error_log( '--Stripe Error | Customer | ApiErrorException: ' . $e->getError()->message );
      return[ 'error' => 'Whoops, like something unexpected happened on our side of things. Please contact subscribe@thebrag.media with the details you submitted.' ]; wp_die();
    } catch (Exception $e) {
  ...
  public function createInvoice( $subtotal, $shipping_cost, $number_of_issues, $coupon_code, $amount_off, $payment_method, $sub_email, $sub_full_name, $buyer = [], $shipping = []) {
    ...
    try {
      ...
      if ( $customer['error'] ) {
        return [ 'error' => $customer['error'] ]; wp_die();
      }
      ...
      return[ 'error' => 'Whoops, it looks like something unexpected happened on our of things. Feel free to refresh your browser and give it another shot.' ]; wp_die();
    } catch (\Stripe\Exception\InvalidRequestException $e) {
