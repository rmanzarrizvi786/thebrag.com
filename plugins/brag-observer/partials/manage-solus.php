...
if (!is_null($action) && 'edit' != $action) {
  switch ($action) {
    case 'preview':
      ...
      break;
    case 'show-html':
      include __DIR__ . '/../partials/show-solus-html.php';
      break;
      /* case 'create-on-mc':
...
<form method="post" action="#" class="create-solus">
  ...
  <table class="table table-sm table-light">
    ...
    <tr>
      <td colspan="4">Subject <small>Max 150 characters</small>
        <input type="text" name="subject" value="<?php echo isset($solus) && isset($solus->details->subject) ? htmlentities($solus->details->subject) : ''; ?>" maxlength="150" class="form-control">
      </td>
    </tr>
    <tr>
      <td colspan="4">Preview Text <small>Max 150 characters</small>
        <input type="text" name="preview_text" value="<?php echo isset($solus) && isset($solus->details->preview_text) ? htmlentities($solus->details->preview_text) : ''; ?>" maxlength="150" class="form-control">
      </td>
    </tr>
    <tr>
      <td>Date<br>
      ...
      <td>Reply to <small>Email Address</small><br>
        <input type="text" name="reply_to" value="<?php echo isset($solus) && isset($solus->details->reply_to) ? $solus->details->reply_to : 'observer@thebrag.media'; ?>" class="form-control">
      </td>

      <!-- <td>From Name<br>
        <input type="text" name="from_name" value="<?php // echo isset($solus) && isset($solus->details->from_name) ? $solus->details->from_name : 'The Brag Observer'; 
  ...
  <div class="mt-3">
    ...
    <input name="solus_link" id="solus_link" type="text" value="<?php echo isset($solus->solus_link) && $solus->solus_link != '' ? stripslashes($solus->solus_link) : ''; ?>" placeholder="https://" class="form-control">
  </div>
