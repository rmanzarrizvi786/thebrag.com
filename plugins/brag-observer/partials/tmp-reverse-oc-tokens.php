...
<form action="<?php echo admin_url( 'admin.php' ); ?>" method="post" class="container-fluid" enctype="multipart/form-data">
  ...
  <div class="container-fluid">
    <h1>Reverse OC Tokens</h1>

    <div class="row">
      <div class="col-md-2 mt-3">
        <button type="button" name="tmp-reverse-oc-tokens" id="tmp-reverse-oc-tokens" class="btn btn-primary">Start</button>
      </div>
  ...
  jQuery(document).ready(function($){
    $('#tmp-reverse-oc-tokens').on('click', function(e) {
      e.preventDefault();
    ...
    function processSubs() {
      $( '#tmp-reverse-oc-tokens' ).prop( 'disabled', true );

      var fd = new FormData();
      fd.append( 'action', 'tmp_reverse_oc_tokens' );
      ...
      $.ajax({
        ...
        success: function(res){
          if ( res.success ) {
            ...
            $( '#tmp-reverse-oc-tokens' ).prop( 'disabled', false );
          }
        },
