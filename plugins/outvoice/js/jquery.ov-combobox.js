...
(function ($, document) {
    ...
    var methods = {
        ...
        fill: function (data, appendMode) {
            ...
            if (!opts.empty) {
                if (mode != 'checkboxes') {
                    ...
                    var selectedValues = $options.filter(':selected').map(
                        function () {
                            return $(this).val();
                        }
                    ).get();
         ...
         */
        clear: function () {
            // TODO check why to or not to remove data itself
         ...
         */
        data: function (data) {
            // this method is required because after setting new options
    ...
    {
        // for checkboxes mode; this refers to combobox
        var $paragraphs = $(this).find(cp + clist + ' p'),
            $vInput = $(this).children(cp + cvalue),
            arrV = [];
        var $lastChecked;
    ...
    function addListeners()
    {
        ...
        this.on(
            'keyup', cp + cdisplay + ', ' + cp + cdiv, function (e) {
                // filter
                // Ignore keys that can't alter input field value on their own
                if ([38, //Up arrow
                    40, //Down arrow
                    13, //Enter                27, //Escape                9, //Tab                37, //Left arrow
                    39, //Right arrow
                    17, //Ctrl                18, //Alt                16, //Shift                20, //Caps lock
                    33, //Page up
                    34, //Page down
                    35, //End                36 //Home
                    ].indexOf(e.which) >= 0
        ...
        this.on(
            'keydown', cp + cdisplay, function (e) {
        ...
        this.on(
            'change', 'select', function (e, checkboxesMode) {
                // someone triggered combobox select change
        ...
        this.on(
            pname + '-chupdate', cp + clist + ' p :checkbox', function (e, forRefresh) {
                if (forRefresh) {
                    e.stopPropagation();
        ...
        this.on(
            'click', cp + clist + ' p', function (e) {
                // value selected by clicking
            ...
            'blur', cp + cdisplay, function (e) {
                ...
                */
                ...
                if (!vOriginal) { // if combo was emptied then set its value to '':
                    ...
                    var value;
                    $t.siblings('select').find('option').each(
                        function () {
                            if (O.filterIgnoreCase) {
        ...
        this.on(
            'click', cp + cdisplay + '-div', function () {
        ...
        $(document).bind(
            'click.' + pname, { thisIs: this }, function (e) {
    ...
    {
        // this refers to paragraph dom element
        var $t = $(this),
            $combo = $t.closest(cp),
            $div = $t.parent(),
            $ps = $div.children('p'),
        ...
        $combo.find(cp + cdholder).fadeOut(
            duration / 5, function () {
