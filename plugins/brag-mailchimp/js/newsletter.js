jQuery(document).ready(function($) {
    ...
    $('.create-campaign #submit-campaign').on('click', function() {
        ...
        var tinymce_content_visual = '';
        var editor_id = tinyMCE.get( 'bannercontent');
        var data = $('.create-campaign').serialize();
        if( editor_id ) {
            if( editor_id.hidden === false ) {
                var tinymce_content_visual = editor_id.getContent();
                var data = data + '&banner_content_visual='+tinymce_content_visual 
            }
        }
        ...
        $.post(
            ...
            $(this).attr('disabled', false).parent().find('.status').html(' Failed to save.');
        });
    });
...
//            if ( $.inArray( post_id, campaign_posts ) === -1 )
    ...
    // Featured Story 3
    $('.create-campaign #add-featured-story-3').autoComplete({
        source: function(name, response) {
            $.ajax({
                type: 'POST',
                dataType: 'json',
        ...
        onSelect: function(e, term, item){
            if ( $('.remove-featured-story-3').length) {
                alert ('You can add only one Featured Story 3, please remove the other one first.');
                return false;
            }
            var post_id = parseInt( item.data('id') );
//            if ( $.inArray( post_id, campaign_posts ) === -1 )
            {
                ...
                var post_thumbnail = item.data('thumbnail');
                var post_link = item.data('link');
                $('#campaign-featured-story-3').append(
                        '<tr id="featured_story_wrap_3">' +
                        '<td width="50"><a href="' + post_link + '" target="_blank"><img src="' + post_thumbnail + '" width="50"></a></td>' +
                        '<td>' +
                        '<label>Link:<input type="text" name="featured_story_link_3" value="' + post_link + '" class="link_remote"></label>' + 
                        '<div class="remote_content">' + 
                        '<label>Title:<input type="text" name="featured_story_title_3" value="' + post_title + '" class="title"></label><br>' +
                        '<label>Blurb:<br><textarea name="featured_story_excerpt_3" class="excerpt">' + post_excerpt + '</textarea></label><br>' +
                        '<label>Image:<br><input type="text" name="featured_story_image_3" value="' + post_thumbnail + '" class="image"></label>' +
                        '</div>' + 
                        '</td>' +
                        '<td><label class="remove remove-featured-story-3" data-id="featured_story_wrap_3">x</label></td>' +
                        '</tr>'
                        );
                $('.create-campaign #add-featured-story-3').val('');
            }
        },
        minChars: 1
    });
    $('.add-featured-story-blank-3').on('click', function(e) {
        e.preventDefault();
        if ( $('.remove-featured-story-3').length) {
            alert ('You can add only one Featured Story 3, please remove the other one first.');
            return false;
        ...
        $('#campaign-featured-story-3').append(
            ...
            '<td width="50">&nbsp;</td>' +
            '<td>' +
            '<label>Link:<input type="text" name="featured_story_link_3" id="featured_story_link_3" value="" class="link_remote"></label>' + 
            '<div class="hide remote_content">' + 
            '<label>Title:<input type="text" name="featured_story_title_3" class="title" value=""></label><br>' +
            '<label>Blurb:<br><textarea name="featured_story_excerpt_3" class="excerpt"></textarea></label><br>' +
            '<label>Image:<br><input type="text" name="featured_story_image_3" class="image" value=""></label>' +
            '</div>' + 
            '</td>' +
            '<td><label class="remove remove-featured-story-3" data-id="featured_story_wrap_3">x</label></td>' +
            '</tr>'
            );
        $('#featured_story_link_3').focus();
    });
...
//                        '<label>Blurb:<br><textarea name="edm_featured_video_excerpt">' + post_excerpt + '</textarea></label>' +
    ...
    $(document).on('click', '.remove-cover-story, .remove-featured-story-1, .remove-featured-story-2, .remove-featured-story-3, .remove-featured-video' , function() {
        $( '#' + $(this).data('id') ).detach();
    });
...
//                    window.location = '?page=brag-mailchimp/brag-mailchimp.php';
    ...
    /*//save banner content
    $( '#banner_content_push_to_template' ).click( function( e ) {
        var tinymce_content_visual, tinymce_content_html = '';
        editor_id = tinyMCE.get( 'addcontentbox');
        template_queue_editor_id = tinyMCE.get( 'addcontenttemplatequeue' );
        if( editor_id ) {
            var tinymce_content_visual = editor_id.getContent();
           
        } else {
            var tinymce_content_html = $( 'textarea[name="banner_textarea"]' ).val();
        }

        
        alert( tinymce_content_visual );
        alert( tinymce_content_html );

        if( template_queue_editor_id && tinymce_content_visual != '' ) {
            editor_id.setContent( tinymce_content_visual );
        } else {
            $( 'textarea[name="banner_textarea_in_template_queue"]').val( tinymce_content_html );
        }
        e.preventDefault();
    });*/
} );