        ...
        <div class="pl-2">
            ...
            foreach ($hero_stories as $story) :
                if ('' !== get_the_post_thumbnail()) :
                    $alt_text = get_post_meta(get_post_thumbnail_id($story->ID), '_wp_attachment_image_alt', true);
                    if ($alt_text == '') {
                        $alt_text = trim(strip_tags(get_the_title()));
                    }
                    $img_src = wp_get_attachment_image_src(get_post_thumbnail_id($story->ID), 'thumbnail');
                endif;
            ?>
